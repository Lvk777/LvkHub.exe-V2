...
return function(State, Registry, UI)
    ...
    local function box3D(color,trans)
        ...
        local idx=1
        for _,e in ipairs(edges) do setLine(box3d[idx],a[e[1]],a[e[2]],1,color,trans); idx+=1 end
        for _,e in ipairs(edges) do setLine(box3d[idx],b[e[1]],b[e[2]],1,color,trans); idx+=1 end
        for i=1,4 do setLine(box3d[idx],a[i],b[i],1,color,trans); idx+=1 end
    end
