-- Local popup V5: settings for client-only Highlight chams and local trail.
return function(State, UI)
    ...
    local function buildPopup(title,builder)
        ...
        ttl.Font=Enum.Font.SourceSansSemibold; ttl.TextSize=14; ttl.TextColor3=Color3.fromRGB(238,238,242)
        ttl.TextXAlignment=Enum.TextXAlignment.Left; ttl.Text=title; ttl.ZIndex=131; ttl.Parent=p

        local close=Instance.new("TextButton")
        close.AnchorPoint=Vector2.new(1,0); close.Position=UDim2.new(1,-7,0,6); close.Size=UDim2.fromOffset(25,22)
        close.BackgroundColor3=Color3.fromRGB(34,35,42); close.BorderSizePixel=0; close.Text="×"
        close.TextColor3=Color3.fromRGB(215,215,222); close.ZIndex=132; close.Parent=p; rounded(close,4)
        ...
        local function base(label,h)
            ...
            r.Position=UDim2.fromOffset(7,y); r.Size=UDim2.new(1,-14,0,h); r.BackgroundColor3=Color3.fromRGB(27,28,34)
            r.BorderSizePixel=0; r.ZIndex=131; r.Parent=p; rounded(r,4)
            local l=Instance.new("TextLabel")
            l.BackgroundTransparency=1; l.Position=UDim2.fromOffset(8,0); l.Size=UDim2.fromOffset(76,h)
            l.Font=Enum.Font.SourceSans; l.TextSize=12; l.TextColor3=Color3.fromRGB(222,222,228)
            l.TextXAlignment=Enum.TextXAlignment.Left; l.Text=label; l.ZIndex=132; l.Parent=r
            y+=h+5
            return r,l
        end

        local function color(label,get,set)
            local r,l=base(label,38)
            l.Size=UDim2.fromOffset(74,38)
            local bar=Instance.new("Frame")
            bar.Position=UDim2.fromOffset(80,11); bar.Size=UDim2.new(1,-90,0,16); bar.BackgroundColor3=Color3.new(1,1,1)
            bar.BorderSizePixel=0; bar.Active=true; bar.ZIndex=133; bar.Parent=r; rounded(bar,4)
            local g=Instance.new("UIGradient"); g.Color=rainbow; g.Parent=bar
            local k=Instance.new("Frame")
            k.AnchorPoint=Vector2.new(.5,.5); k.Size=UDim2.fromOffset(4,22); k.BackgroundColor3=Color3.fromRGB(248,248,250)
            k.BorderSizePixel=0; k.ZIndex=134; k.Parent=bar
        ...
        local function slider(label,get,set,min,max,step)
            local r,l=base(label,38)
            l.Size=UDim2.fromOffset(72,38)
            local bar=Instance.new("Frame")
            bar.Position=UDim2.fromOffset(78,16); bar.Size=UDim2.new(1,-133,0,6); bar.BackgroundColor3=Color3.fromRGB(43,44,51)
            bar.BorderSizePixel=0; bar.Active=true; bar.ZIndex=133; bar.Parent=r; rounded(bar,3)
            ...
            local function paint()
                ...
                local a=(n-min)/math.max(.001,max-min)
                fill.Size=UDim2.new(a,0,1,0); k.Position=UDim2.new(a,0,.5,0)
            ...
            local function setX(x)
                ...
                local n=min+(max-min)*a; n=math.floor(n/step+.5)*step
                set(math.clamp(n,min,max)); paint()
            end
            bar.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true; setX(i.Position.X) end end)
            k.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true; setX(i.Position.X) end end)
            UIS.InputChanged:Connect(function(i) if dragging and i.UserInputType==Enum.UserInputType.MouseMovement then setX(i.Position.X) end end)
            UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dragging=false end end)
            paint()
        end
        ...
        local function toggle(label,get,set)
            ...
            b.Text=""; b.BorderSizePixel=0; b.ZIndex=133; b.Parent=r; rounded(b,3)
    ...
    local function addDots(labels,open)
        local r,t=rowByText(labels)
        if not r or r:FindFirstChild("LvkLocalDotsV5") then return end
        t.Size=UDim2.new(1,-78,1,0)
        local d=Instance.new("TextButton")
        d.Name="LvkLocalDotsV5"; d.AnchorPoint=Vector2.new(1,.5); d.Position=UDim2.new(1,-44,.5,0)
        d.Size=UDim2.fromOffset(26,20); d.BackgroundColor3=Color3.fromRGB(34,34,40); d.BorderSizePixel=0
        d.Font=Enum.Font.SourceSansBold; d.TextSize=15; d.TextColor3=Color3.fromRGB(195,195,205); d.Text="•••"; d.Parent=r; rounded(d,4)
        d.MouseButton1Click:Connect(open)
    end
