...
return function(State, Registry, UI)
    ...
    local function chooseTarget(requireVisible)
        -- ALTERADO: itera sobre Players reais
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            if plr ~= LP and model and Registry.IsBot(model) then
    ...
    local function restoreHitboxes()
        ...
        local folder = Workspace:FindFirstChild("Players") -- ALTERADO: Players
        if folder then
    ...
    local _, sourceLabel = UI.Row(page, "Target source: Workspace.Players") -- ALTERADO
    ...
    local function updateCount()
        countLabel.Text = "Test targets: " .. tostring(Registry.CountBots and Registry.CountBots() or 0) .. " • real Players included" -- ALTERADO
    end
    updateCount()
    ...
    local function setGuarded(key, v)
        -- ALTERADO: sem restrição
        State.Combat[key] = v and true or false
