-- BulletTracer V3: local cosmetic beam with multiple local shot signals.
-- It never sends remotes or changes weapon/server state.
return function(State, Registry, UI)
    ...
    local C=State.Local._BulletTracerV3 or State.Local._BulletTracerV2 or {
        Color=Color3.fromRGB(119,120,255),Transparency=5,Lifetime=.18,Thickness=.08,Glow=true,Distance=20000,
    }
    State.Local._BulletTracerV3=C
    ...
    local rainbow=ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(4/6,Color3.fromRGB(0,0,255)),ColorSequenceKeypoint.new(5/6,Color3.fromRGB(255,0,255)),ColorSequenceKeypoint.new(1,Color3.fromRGB(255,0,0)),
    })

    local function options()
        local p=Instance.new("Frame"); p.Name="LvkBulletTracerOptionsV3"; p.Size=UDim2.fromOffset(236,252); p.BackgroundColor3=Color3.fromRGB(17,18,22); p.BorderSizePixel=0; p.ZIndex=130; rounded(p,7)
        ...
        local function slider(label,get,set,min,max,step,y)
            ...
            bar.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then drag=true; setX(i.Position.X) end end); k.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then drag=true; setX(i.Position.X) end end)
            UIS.InputChanged:Connect(function(i) if drag and i.UserInputType==Enum.UserInputType.MouseMovement then setX(i.Position.X) end end); UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then drag=false end end); paint()
        end
        ...
        local cb=Instance.new("Frame"); cb.Position=UDim2.fromOffset(80,11); cb.Size=UDim2.new(1,-90,0,16); cb.BackgroundColor3=Color3.new(1,1,1); cb.BorderSizePixel=0; cb.Active=true; cb.ZIndex=133; cb.Parent=cr; rounded(cb,4); local grad=Instance.new("UIGradient"); grad.Color=rainbow; grad.Parent=cb
        ...
        local cd=false; local function colorX(x) local h=math.clamp((x-cb.AbsolutePosition.X)/math.max(1,cb.AbsoluteSize.X),0,1); C.Color=Color3.fromHSV(h,1,1); ck.Position=UDim2.new(h,0,.5,0) end
        cb.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then cd=true;colorX(i.Position.X) end end); UIS.InputChanged:Connect(function(i) if cd and i.UserInputType==Enum.UserInputType.MouseMovement then colorX(i.Position.X) end end); UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then cd=false end end); ck.Position=UDim2.new(select(1,C.Color:ToHSV()),0,.5,0)
        ...
        local gb=Instance.new("TextButton"); gb.AnchorPoint=Vector2.new(1,.5); gb.Position=UDim2.new(1,-7,.5,0); gb.Size=UDim2.fromOffset(34,18); gb.Text=""; gb.BorderSizePixel=0; gb.ZIndex=133; gb.Parent=gr; rounded(gb,3); local function gp() gb.BackgroundColor3=C.Glow and UI.Accent or Color3.fromRGB(48,49,57) end; gb.MouseButton1Click:Connect(function() C.Glow=not C.Glow;gp() end);gp()
    ...
    tog.MouseButton1Click:Connect(flip); text.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then flip() end end); dots.MouseButton1Click:Connect(options);paint()

    local old=Workspace:FindFirstChild("LvkHubBulletTracersV3"); if old then old:Destroy() end
    local folder=Instance.new("Folder"); folder.Name="LvkHubBulletTracersV3"; folder.Parent=Workspace
    local function worldPos(o) if o and o:IsA("Attachment") then return o.WorldPosition elseif o and o:IsA("BasePart") then return o.Position end end
    ...
    local function muzzle(tool)
        local p=worldPos(tool and tool:FindFirstChild("Muzzle",true)); if p then return p end
        ...
        if tool then
            ...
            for _,m in ipairs(cam:GetDescendants()) do
                if m:IsA("Model") then
                    local n=string.lower(m.Name); local mm=m:FindFirstChild("Muzzle",true)
                    if mm and (n==tn or n:find(tn,1,true) or m:FindFirstChild("Grip",true)) then p=worldPos(mm); if p then return p end end
                end
            end
        end
    ...
    local function endpoint()
        local cam=Workspace.CurrentCamera; if not cam then return nil end
        local center=cam.ViewportSize/2; local ray=cam:ViewportPointToRay(center.X,center.Y)
        local rp=RaycastParams.new(); rp.FilterType=Enum.RaycastFilterType.Exclude; local ex={folder,cam}; if LP.Character then table.insert(ex,LP.Character) end; rp.FilterDescendantsInstances=ex;rp.IgnoreWater=true
        local hit=Workspace:Raycast(ray.Origin,ray.Direction*(C.Distance or 20000),rp); return hit and hit.Position or (ray.Origin+ray.Direction*(C.Distance or 20000))
    end
    local function point(v) local p=Instance.new("Part"); p.Size=Vector3.new(.03,.03,.03);p.Transparency=1;p.Anchored=true;p.CanCollide=false;p.CanTouch=false;p.CanQuery=false;p.CFrame=CFrame.new(v);p.Parent=folder; local a=Instance.new("Attachment");a.Parent=p;return p,a end
    local lastSpawn=0
    local function spawn(tool)
        if not State.Local.BulletTracer or not tool or tool.Parent~=LP.Character then return end
        local now=os.clock(); if now-lastSpawn<.035 then return end; lastSpawn=now
        local a=muzzle(tool); local b=endpoint(); if not a or not b then return end
        local p0,a0=point(a); local p1,a1=point(b); local beam=Instance.new("Beam"); beam.Attachment0=a0;beam.Attachment1=a1;beam.FaceCamera=true;beam.Color=ColorSequence.new(C.Color);beam.Transparency=NumberSequence.new(math.clamp((C.Transparency or 0)/100,0,1));beam.Width0=C.Thickness or .08;beam.Width1=C.Thickness or .08;beam.LightEmission=C.Glow and 1 or .2;beam.LightInfluence=0;beam.Parent=p0
        if C.Glow then local g=beam:Clone();g.Name="Glow";g.Width0=(C.Thickness or .08)*2.8;g.Width1=g.Width0;g.Transparency=NumberSequence.new(math.clamp((C.Transparency or 0)/100+.45,0,1));g.Parent=p0 end
    ...
    local function watchTool(tool)
        ...
        watched[tool]=true
        local ammo=tool:FindFirstChild("Ammo"); local mag=ammo and ammo:FindFirstChild("MagAmmo"); local last=mag and tonumber(mag.Value)
        if mag and mag:IsA("ValueBase") then mag:GetPropertyChangedSignal("Value"):Connect(function() local n=tonumber(mag.Value); if n and last and n<last then spawn(tool) end; last=n end) end
        tool.Activated:Connect(function() task.defer(spawn,tool) end)
    end
    local function scan()
        local ch=LP.Character; if ch then for _,o in ipairs(ch:GetChildren()) do watchTool(o) end end
        local bp=LP:FindFirstChildOfClass("Backpack"); if bp then for _,o in ipairs(bp:GetChildren()) do watchTool(o) end end
    end
    scan()
    if LP.Character then LP.Character.ChildAdded:Connect(function(o) task.defer(watchTool,o) end) end
    local bp=LP:FindFirstChildOfClass("Backpack"); if bp then bp.ChildAdded:Connect(function(o) task.defer(watchTool,o) end) end
    LP.CharacterAdded:Connect(function(ch) ch.ChildAdded:Connect(function(o) task.defer(watchTool,o) end);task.delay(.25,scan) end)
    UIS.InputBegan:Connect(function(i,processed)
        if i.UserInputType==Enum.UserInputType.MouseButton1 and State.Local.BulletTracer then
            local tool=currentWeapon(); if tool then task.defer(spawn,tool) end
        end
    end)
end
