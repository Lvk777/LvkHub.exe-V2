-- BulletTracer V8: local cosmetic tracer emitted only from actual WeaponShotBuilder shots.
-- Requires an equipped weapon and never falls back to camera/mouse-click emission.
return function(State, Registry, UI)
    ...
    local old=State.Local._BulletTracerV7
    local C=State.Local._BulletTracerV8 or old or {
        Color=Color3.fromRGB(119,120,255),Transparency=5,Lifetime=.20,Thickness=.08,Glow=true,Distance=20000,
    }
    State.Local._BulletTracerV8=C
    ...
    local function currentWeapon()
        ...
        for _,o in ipairs(ch:GetChildren()) do
            if o:IsA("Tool") and (o:FindFirstChild("WeaponConfig") or o:FindFirstChild("Ammo") or o:GetAttribute("InventoryManagedTool")~=nil) then
                return o
            end
        end
        return nil
    ...
    local function findMuzzle(root)
        ...
        local exact=root:FindFirstChild("Muzzle",true)
        local p=worldPos(exact)
        if p then return p end

        for _,o in ipairs(root:GetDescendants()) do
            local n=string.lower(o.Name)
            if (o:IsA("Attachment") or o:IsA("BasePart")) and (n:find("muzzle",1,true) or n=="barrelend" or n=="barrel_end") then
                p=worldPos(o)
                if p then return p end
            end
        end
        ...
        if grip and grip:IsA("BasePart") then
            return grip.Position+grip.CFrame.LookVector*math.max(.45,grip.Size.Z*.5)
        end
        local handle=root:FindFirstChild("Handle",true)
        if handle and handle:IsA("BasePart") then
            return handle.Position+handle.CFrame.LookVector*math.max(.45,handle.Size.Z*.5)
        end
        local barrel=root:FindFirstChild("Barrel",true)
        if barrel and barrel:IsA("BasePart") then
            return barrel.Position+barrel.CFrame.LookVector*math.max(.45,barrel.Size.Z*.5)
        end
        return nil
    end
    ...
    local function firstPerson()
        ...
        return (cam.CFrame.Position-cam.Focus.Position).Magnitude<1.25
    end

    local function muzzlePosition()
        local tool=currentWeapon()
        if not tool then return nil end -- hard requirement: no equipped gun = no tracer

        local cam=Workspace.CurrentCamera
        if firstPerson() and cam then
            -- First person: use the visible viewmodel muzzle when one exists.
            for _,o in ipairs(cam:GetChildren()) do
                if o:IsA("Model") or o:IsA("Folder") then
                    local p=findMuzzle(o)
                    if p then return p end
                end
            end
        end

        -- Third person (and first-person fallback): actual equipped Tool muzzle.
        return findMuzzle(tool)
    end

    for _,name in ipairs({"LvkHubBulletTracersV6","LvkHubBulletTracersV7","LvkHubBulletTracersV8"}) do
        local o=Workspace:FindFirstChild(name)
        if o then o:Destroy() end
    end
    local folder=Instance.new("Folder")
    folder.Name="LvkHubBulletTracersV8"
    folder.Parent=Workspace

    local function anchor(pos)
        local p=Instance.new("Part")
        p.Size=Vector3.new(.02,.02,.02)
        p.Transparency=1
        p.Anchored=true
        p.CanCollide=false
        p.CanTouch=false
    ...
    local function selectedEndpoint()
        ...
        if m and Registry.IsBot(m) then
            local p=(State.Combat.AimPart=="Torso" and (m:FindFirstChild("UpperTorso") or m:FindFirstChild("Torso")))
                or m:FindFirstChild("Head") or Registry.RootOf(m)
    ...
    local function emit(direction)
        ...
        local now=os.clock()
        if now-lastEmit<.02 then return end
        lastEmit=now
        ...
        local p0,a0=anchor(from)
        local p1,a1=anchor(to)
        local beam=Instance.new("Beam")
        beam.Attachment0=a0
        beam.Attachment1=a1
        beam.FaceCamera=true
        beam.Color=ColorSequence.new(C.Color)
        beam.Transparency=NumberSequence.new(math.clamp((C.Transparency or 0)/100,0,1))
        beam.Width0=C.Thickness or .08
        beam.Width1=C.Thickness or .08
        beam.LightEmission=C.Glow and 1 or 0
        beam.LightInfluence=C.Glow and 0 or 1
        beam.Parent=p0
        if C.Glow then
            local glow=beam:Clone()
            glow.Width0=(C.Thickness or .08)*3
            glow.Width1=glow.Width0
            glow.Transparency=NumberSequence.new(.72)
            glow.Parent=p0
        end
        local life=math.clamp(tonumber(C.Lifetime) or .20,.05,1.5)
        Debris:AddItem(p0,life)
        Debris:AddItem(p1,life)
    end

    ----------------------------------------------------------------------
    -- Compact Yokai-style row + options.
    ----------------------------------------------------------------------
    local rainbow=ColorSequence.new({
        ColorSequenceKeypoint.new(0,Color3.fromRGB(255,0,0)),ColorSequenceKeypoint.new(.17,Color3.fromRGB(255,255,0)),
        ColorSequenceKeypoint.new(.34,Color3.fromRGB(0,255,0)),ColorSequenceKeypoint.new(.5,Color3.fromRGB(0,255,255)),
        ColorSequenceKeypoint.new(.67,Color3.fromRGB(0,0,255)),ColorSequenceKeypoint.new(.84,Color3.fromRGB(255,0,255)),
        ColorSequenceKeypoint.new(1,Color3.fromRGB(255,0,0)),
    })

    local function openOptions()
        local p=Instance.new("Frame")
        p.Name="LvkBulletTracerOptionsV8"
        p.Size=UDim2.fromOffset(236,252)
        ...
        ttl.BackgroundTransparency=1;ttl.Position=UDim2.fromOffset(10,5);ttl.Size=UDim2.new(1,-42,0,28)
        ttl.Font=Enum.Font.SourceSansSemibold;ttl.TextSize=14;ttl.TextColor3=Color3.fromRGB(238,238,242);ttl.TextXAlignment=Enum.TextXAlignment.Left;ttl.Text="BulletTracer";ttl.ZIndex=131;ttl.Parent=p
        local close=Instance.new("TextButton")
        close.AnchorPoint=Vector2.new(1,0);close.Position=UDim2.new(1,-7,0,6);close.Size=UDim2.fromOffset(25,22)
        close.BackgroundColor3=Color3.fromRGB(34,35,42);close.BorderSizePixel=0;close.Text="×";close.TextColor3=Color3.fromRGB(215,215,222);close.ZIndex=132;close.Parent=p;round(close,4)
        ...
        local function base(label)
            ...
            local l=Instance.new("TextLabel");l.BackgroundTransparency=1;l.Position=UDim2.fromOffset(8,0);l.Size=UDim2.fromOffset(70,38);l.Font=Enum.Font.SourceSans;l.TextSize=12;l.TextColor3=Color3.fromRGB(222,222,228);l.TextXAlignment=Enum.TextXAlignment.Left;l.Text=label;l.ZIndex=132;l.Parent=r
            y+=43
            return r,l
        end

        local r=base("Color")
        local bar=Instance.new("Frame");bar.Position=UDim2.fromOffset(78,11);bar.Size=UDim2.new(1,-88,0,16);bar.BackgroundColor3=Color3.new(1,1,1);bar.BorderSizePixel=0;bar.Active=true;bar.ZIndex=133;bar.Parent=r;round(bar,4)
        ...
        local function colorX(x)
            ...
            C.Color=Color3.fromHSV(h,1,1);k.Position=UDim2.new(h,0,.5,0)
        end
        bar.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then drag=true;colorX(i.Position.X) end end)
        UIS.InputChanged:Connect(function(i) if drag and i.UserInputType==Enum.UserInputType.MouseMovement then colorX(i.Position.X) end end)
        UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then drag=false end end)
        k.Position=UDim2.new(select(1,C.Color:ToHSV()),0,.5,0)

        local function slider(label,get,set,min,max,step)
            local rr,ll=base(label)
            local b=Instance.new("Frame");b.Position=UDim2.fromOffset(76,16);b.Size=UDim2.new(1,-131,0,6);b.BackgroundColor3=Color3.fromRGB(43,44,51);b.BorderSizePixel=0;b.Active=true;b.ZIndex=133;b.Parent=rr;round(b,3)
            ...
            local function paint()
                ...
                fill.Size=UDim2.new(a,0,1,0);kk.Position=UDim2.new(a,0,.5,0)
                val.Text=step<1 and string.format("%.2f",n) or tostring(math.floor(n+.5))
            end
            local function fromX(x)
                local a=math.clamp((x-b.AbsolutePosition.X)/math.max(1,b.AbsoluteSize.X),0,1)
                local n=min+(max-min)*a;n=math.floor(n/step+.5)*step;set(math.clamp(n,min,max));paint()
            end
            b.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then d=true;fromX(i.Position.X) end end)
            kk.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then d=true;fromX(i.Position.X) end end)
            UIS.InputChanged:Connect(function(i) if d and i.UserInputType==Enum.UserInputType.MouseMovement then fromX(i.Position.X) end end)
        ...
        local gr=base("Glow")
        local gb=Instance.new("TextButton");gb.AnchorPoint=Vector2.new(1,.5);gb.Position=UDim2.new(1,-7,.5,0);gb.Size=UDim2.fromOffset(34,18);gb.Text="";gb.BorderSizePixel=0;gb.ZIndex=133;gb.Parent=gr;round(gb,3)
    ...
    local function paint()
        local on=State.Local.BulletTracer
    ...
    ----------------------------------------------------------------------
    -- Actual-shot hook only. No InputBegan fallback.
    ----------------------------------------------------------------------
    ...
    local function install()
        ...
        local ok,b=pcall(require,mod)
        if not ok or type(b)~="table" or type(b.GetSpreadDirection)~="function" then return false end
        ...
        b.GetSpreadDirection=function(baseDirection,spreadState)
            ...
            if currentWeapon() then
                task.defer(emit,result or baseDirection)
            end
            return result
        end
        installed=true
