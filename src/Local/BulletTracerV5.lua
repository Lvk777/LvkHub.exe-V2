-- BulletTracer V5: local cosmetic tracer only.
-- One tracer per local shot, from the actual weapon/viewmodel muzzle to the point aimed at.
-- Does not send remotes, alter bullets, or change server weapon state.
return function(State, Registry, UI)
    ...
    local C=State.Local._BulletTracerV5 or State.Local._BulletTracerV4 or {
    ...
    local function openOptions()
        ...
        p.Name="LvkBulletTracerOptionsV5"
        ...
        title.Font=Enum.Font.SourceSansSemibold;title.TextSize=14;title.TextColor3=Color3.fromRGB(238,238,242)
        title.TextXAlignment=Enum.TextXAlignment.Left;title.Text="BulletTracer";title.ZIndex=131;title.Parent=p
        local close=Instance.new("TextButton")
        close.AnchorPoint=Vector2.new(1,0);close.Position=UDim2.new(1,-7,0,6);close.Size=UDim2.fromOffset(25,22)
        close.BackgroundColor3=Color3.fromRGB(34,35,42);close.BorderSizePixel=0;close.Text="×";close.TextColor3=Color3.fromRGB(215,215,222);close.ZIndex=132;close.Parent=p;rounded(close,4)
        ...
        local cr,cl=row("Color",39)
        cl.Size=UDim2.fromOffset(74,38)
    ...
    -- Tracer runtime.
    ------------------------------------------------------------------------
    for _,name in ipairs({"LvkHubBulletTracersV2","LvkHubBulletTracersV3","LvkHubBulletTracersV4","LvkHubBulletTracersV5"}) do
        local old=Workspace:FindFirstChild(name);if old then old:Destroy() end
    end
    local folder=Instance.new("Folder");folder.Name="LvkHubBulletTracersV5";folder.Parent=Workspace
    ...
    local function muzzleObject(root)
        if not root then return nil end
        local exact={muzzle=true,muzzlepoint=true,muzzleattachment=true,barrelend=true,firepoint=true,tip=true}
        for _,d in ipairs(root:GetDescendants()) do
            if d:IsA("Attachment") or d:IsA("BasePart") then
                local n=d.Name:lower():gsub("[%s_%-]","")
                if exact[n] or n:find("muzzle",1,true) then return d end
            end
        end
        local grip=root:FindFirstChild("Grip",true)
        if grip then
            local m=grip:FindFirstChild("Muzzle",true)
            if m and (m:IsA("Attachment") or m:IsA("BasePart")) then return m end
        end
    end

    local function muzzlePosition(tool)
        local cam=Workspace.CurrentCamera
        -- Prefer a first-person viewmodel if one exists and visibly owns a muzzle.
        if cam then
            ...
            for _,m in ipairs(cam:GetDescendants()) do
                if m:IsA("Model") then
                    ...
                    if toolName=="" or n==toolName or n:find(toolName,1,true) or m:FindFirstChild("Grip",true) then
                        local obj=muzzleObject(m)
                        if obj then return worldPosition(obj) end
                    end
                end
            end
        end
        local obj=muzzleObject(tool)
        if obj then return worldPosition(obj) end
    ...
    local function aimPoint()
        ...
        rp.IgnoreWater=true
        local distance=tonumber(C.Distance) or 20000
    ...
    local function makeAnchor(position)
        ...
        p.Size=Vector3.new(.02,.02,.02);p.Transparency=1;p.Anchored=true;p.CanCollide=false;p.CanTouch=false;p.CanQuery=false
        p.CFrame=CFrame.new(position);p.Parent=folder
    ...
    local function emit(tool)
        ...
        local now=os.clock()
        if now-lastTrace<.035 then return end
        lastTrace=now
        tool=tool or currentWeapon()
        if not tool then return end
        local from=muzzlePosition(tool)
        local to=aimPoint()
        if not from or not to or (to-from).Magnitude<.25 then return end

        local p0,a0=makeAnchor(from)
        local p1,a1=makeAnchor(to)
        local beam=Instance.new("Beam")
        beam.Name="LvkHubTracer"
        beam.Attachment0=a0;beam.Attachment1=a1
        beam.FaceCamera=true
        beam.Width0=math.max(.01,tonumber(C.Thickness) or .08)
        beam.Width1=beam.Width0
        beam.Color=ColorSequence.new(C.Color,C.Color)
        beam.Transparency=NumberSequence.new(math.clamp((tonumber(C.Transparency) or 0)/100,0,1))
        beam.LightEmission=C.Glow and 1 or .15
        beam.LightInfluence=C.Glow and 0 or 1
        beam.Parent=p0
        local life=math.clamp(tonumber(C.Lifetime) or .18,.05,2)
    ...
    local function watchTool(tool)
        ...
        if mag and mag:IsA("ValueBase") then
            ...
            mag:GetPropertyChangedSignal("Value"):Connect(function()
                ...
                if n and last and n<last then emit(tool) end
                last=n
            end)
        else
            tool.Activated:Connect(function() emit(tool) end)
        end
    end
    ...
    local function bindContainers()
        local ch=LP.Character
        if ch then ch.ChildAdded:Connect(function(o) if o:IsA("Tool") then task.defer(watchTool,o) end end) end
        local bp=LP:FindFirstChildOfClass("Backpack")
        if bp then bp.ChildAdded:Connect(function(o) if o:IsA("Tool") then task.defer(watchTool,o) end end) end
    end

    LP.CharacterAdded:Connect(function() task.delay(.2,function() bindContainers();scanTools() end) end)
    bindContainers();scanTools()
end
